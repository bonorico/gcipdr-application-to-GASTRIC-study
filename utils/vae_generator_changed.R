...
## keras installation note 30.6.20 (system info: Ubuntu 20.04 focal LTS, R version 4.0.2 (2020-06-22) -- "Taking Off Again"). You might encounter the 'ERROR: tensorflow 2.2.0 has requirement scipy==1.4.1; python_version >= "3", but you'll have scipy 1.5.0 which is incompatible.' Just manually do library(keras); install_keras()
...
VAE <- function(x_train, H, epochs = 30L, repeat_data = 100, batch_split = 0.3){
    ...
    x_train_augm <-  do.call("rbind", lapply(1:repeat_data, function(i){  ## data augmentation
        ...
        out[, !isbin] <- apply(as.matrix(x_train[, !isbin]), 2, function(x) x + rnorm(orig_dim[1], sd = 0.01))  
